...
import { ArrowLeft, Mail, CheckCircle2, AlertCircle, User } from 'lucide-react'
        ...
        <div className="min-h-screen bg-slate-100 flex items-center justify-center font-outfit p-6">
            ...
            <div ref={cardRef} className="bg-white rounded-[2rem] shadow-2xl p-10 max-w-md w-full border border-white/50">
                ...
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-6 transition-all duration-500 ${sent ? 'bg-green-50' : 'bg-slate-900'}`}>
                    {sent ? <CheckCircle2 size={32} className="text-green-500" /> : <User size={28} className="text-white" />}
                </div>
                ...
                {sent ? (
                    <div className="text-center">
                        <h1 className="text-2xl font-black text-slate-900 mb-3">Email Sent!</h1>
                        <p className="text-slate-500 text-sm leading-relaxed mb-8">
                            If an account with <strong className="text-slate-800">{email}</strong> exists,
                            we've sent your username to that address. Check your inbox.
                        </p>
                        <div className="bg-slate-50 rounded-2xl p-4 text-left space-y-2 mb-8 border border-slate-100">
                            {['Open your email inbox', 'Look for an email from Mocker Platform', 'Use the username shown to log in'].map((step, i) => (
                    ...
                    <>
                        <h1 className="text-2xl font-black text-slate-900 mb-1 text-center">Forgot Username?</h1>
                        ...
                        <div className="mt-6 pt-6 border-t border-slate-100 text-center">
                            <p className="text-[10px] text-slate-400 font-medium">
                                ...
                                <Link to="/forgot-password" className="text-slate-900 font-black hover:underline">
                                    Reset Password
                                </Link>
                            </p>
                        </div>
